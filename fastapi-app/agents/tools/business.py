...
from agents.tools.product import check_stock, recommend_cake
...
def business_tools() -> list[BaseTool]:
    ...
    async def cancel_order_tool(
        ...
        if not any(word in context.user_message.lower() for word in ("取消", "撤销", "退订", "不要了", "cancel")):
    ...
    async def recommend_cake_tool(
        ...
        try:
            return await recommend_cake(preference=preference)
        except Exception:
