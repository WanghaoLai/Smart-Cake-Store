...
from agents.agent import AgentUnavailableError, LangChainAgentExecutor
...
def make_executor(graph=None, max_history=2, configured=True, grounding_service=None):
...
class LangChainAgentExecutorTests(unittest.IsolatedAsyncioTestCase):
    async def test_invokes_graph_with_bounded_langchain_messages_and_trusted_context(self):
        graph = FakeAgentGraph("已完成")
    ...
    async def test_model_credentials_are_checked_before_graph_invocation(self):
        graph = FakeAgentGraph()
        executor = make_executor(graph, configured=False)
        with self.assertRaises(AgentUnavailableError):
            await executor.process_message("hello", [])
        self.assertEqual(graph.calls, [])

    async def test_graph_failure_is_wrapped_without_exposing_provider_error(self):
        executor = make_executor(FakeAgentGraph(error=RuntimeError("secret provider detail")))
    ...
    async def test_invalid_or_empty_final_message_is_rejected(self):
        executor = make_executor(FakeAgentGraph(result={"messages": [HumanMessage(content="hello")]}))
    ...
    async def test_public_stream_contains_only_final_answer(self):
        ...
        executor = make_executor(FakeAgentGraph(answer))
    ...
    async def test_server_evidence_is_inserted_before_current_user_message(self):
        ...
        graph = FakeAgentGraph("已根据库存回答")
        grounding = FakeGrounding()
        executor = make_executor(graph, grounding_service=grounding)
        with patch(
            "agents.agent.executor.business_repository.validate_product_answer",
            new=AsyncMock(return_value=True),
        ):
            await executor.process_message("草莓蛋糕还有货吗", [], user_id=7)

        messages = graph.calls[0]["input"]["messages"]
    ...
    async def test_hallucinated_catalog_answer_is_replaced_with_verified_mysql_answer(self):
        ...
        graph = FakeAgentGraph("1. 经典红丝绒蛋糕 - ¥98（库存 12份）")
        executor = make_executor(graph, grounding_service=FakeGrounding())
        with patch(
            "agents.agent.executor.business_repository.validate_product_answer",
            new=AsyncMock(return_value=False),
        ), patch(
            "agents.agent.executor.business_repository.build_verified_product_answer",
