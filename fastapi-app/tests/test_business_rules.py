...
class BusinessRulesTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def _place_order(self, num: int = 1) -> int:
        payload = OrdersCreatePydantic(goodsId=1, num=num)
    ...
    async def test_order_rejects_oversell_and_missing_goods(self):
        with self.assertRaises(CustomException):
            await order_add(OrdersCreatePydantic(goodsId=1, num=11), USER)
        with self.assertRaises(CustomException):
            await order_add(OrdersCreatePydantic(goodsId=999, num=1), USER)
