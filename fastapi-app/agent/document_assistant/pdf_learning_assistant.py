...
# print(f"[耗时] 模块导入: {time.time() - _t_import_start:.2f}s")
class PDFLearningAssistant:
    ...
    def __init__(self, user_id: str = "default_user"):
        ...
        print(f"[耗时] MemoryTool初始化: {time.time() - t0:.2f}s")
        t0 = time.time()
        self.rag_tool = RAGTool(rag_namespace=f"pdf_{user_id}")
        print(f"[耗时] RAGTool初始化: {time.time() - t0:.2f}s")
    ...
    def load_document(self, pdf_path: str) -> Dict[str, Any]:
        ...
        result = self.rag_tool.execute(
            "add_document",  # 操作类型："add_document" - 添加文档到数据库
            file_path=pdf_path,  # 文件路径：file_path - PDF文件的路径
            chunk_size=1000,  # 分块参数：chunk_size=1000，chunk_overlap=200 - 控制文本分块
            chunk_overlap=200,
        )
    ...
    def recall(self, query: str, limit: int = 5) -> str:
        """回顾学习历程"""
        result = self.memory_tool.execute("search", query=query, limit=limit)
        return result
    ...
    def generate_report(self, save_to_file: bool = True) -> Dict[str, Any]:
        ...
        if save_to_file:
            report_file = f"learned_report_{self.session_id}.json"
...
def main():
    ...
    print(f"[耗时] create_gradio_ui: {time.time() - t0:.2f}s")
    print("demo创建成功！")
    t1 = time.time()
    demo.launch(
        server_name="0.0.0.0",
    ...
    print(f"[耗时] demo.launch: {time.time() - t1:.2f}s")
