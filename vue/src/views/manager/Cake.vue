  ...
  <div class="cake-page">
    ...
    <el-dialog v-model="data.formVisible" width="480px" :close-on-click-modal="false" destroy-on-close class="reserve-dialog">
      ...
      <el-form ref="formRef" :model="data.form" :rules="rules" label-position="top">
        <el-form-item label="规格" v-if="parseSpecs(data.tableData.find(g => g.id === data.form.goodsId)?.specs).length">
          <el-select v-model="data.form.spec">
            <el-option v-for="spec in parseSpecs(data.tableData.find(g => g.id === data.form.goodsId)?.specs)" :key="spec" :label="spec" :value="spec" />
          </el-select>
        </el-form-item>
...
const loadFavorites = () => {
  request.get('/favorite/list').then(res => {
    if (res.code === '200') {
      const map = {}
      ;(res.data?.list || []).forEach(g => { map[g.id] = true })
...
const reserveInit = (goodsId) => {
  ...
  data.form = {
    userId: data.user.id,
    goodsId,
    spec: parseSpecs(data.tableData.find(g => g.id === goodsId)?.specs)[0] || "",
    num: 1,
