...
export const useCartStore = defineStore('cart', {
  ...
  actions: {
    async loadCount() {
      ...
      const ownerKey = `${user.role || ''}:${user.id || ''}`
      try {
        const res = await request.get('/cart/count')
        const current = JSON.parse(localStorage.getItem('system-user') || '{}')
        if (ownerKey === `${current.role || ''}:${current.id || ''}` && res.code === '200') {
    ...
    async addGoods(goodsId, num = 1, spec = "") {
      const res = await request.post('/cart/add', { goodsId, num, spec })
    ...
    async checkout(ids, addressId) {
      const res = await submitPurchase('/cart/checkout', { ids: [...ids].sort((a, b) => a - b), addressId })
      if (res.code === '200') {
        this.items = this.items.filter(i => !ids.includes(i.id))
        this.loadCount()
      }
